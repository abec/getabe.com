...
;(function(factory) {
  ...
  window.VAD = function(config) {
     ...
     * @param  {array} buffer frames of the signal.
     * @return {int} Number of times the signal crosses zero.
     */
    function calculateZeroCrossingRate(buffer) {
      ...
      var count = 0,
          direction = (buffer[i] > 0) ? true : false;

      for (var i = 0; i < buffer; ++i) {
        if (direction && buffer[i] < 0) {
          direction = false;
          count++;
        } else if (!direction && buffer[i] >= 0) {
          direction = true;
          count++;
        }
      }
    ...
    /**
     * Autocorrelation coefficient as defined: http://en.wikipedia.org/wiki/Autocorrelation.
     * @param  {array} buffer frames of the signal.
     * @return {int} Autcorrelation coefficient.
     */
    function calculateAutocorrelationCoefficient(buffer) {
      var top = 0,
          bottom = 0,
          std1 = 0,
          std2 = 0,
      ...
      for (var i = 1; i < buffer.length; ++i) {
        top += buffer[i - 1] * buffer[i];
        std1 += Math.pow(buffer[i - 1], 2);
        std2 += Math.pow(buffer[i], 2);
      }
      bottom = Math.sqrt(std1 * std2);
      return top / bottom;
    }
