...
function uni2pinyin(string) {
  console.debug(string);
  var newSequence = [];
  for (var i = 0, len = string.length; i < len; ++i) {
    var hexn = string.charCodeAt(i).toString(16).toUpperCase();
    if (hexn in UNICHAR2PINYIN) {{
      newSequence.push(UNICHAR2PINYIN[hexn].split(' ')[0]);
    }}
  }
  var pinyinnumbers = newSequence.join(' ');
  console.debug(pinyinnumbers);
  var pinyintones = PinyinConverter.convert(pinyinnumbers);
  console.debug(pinyintones);
  return pinyintones;
}
